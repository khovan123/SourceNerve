...
The Fogewise VPS already uses Caddy. Keep SourceNerve bound to loopback:
...
Append that block to the existing `/etc/caddy/Caddyfile`, then validate and reload:
...
The site block proxies `sourcenerve.fogewise.io.vn` to `127.0.0.1:7331` and disables response buffering for Streamable HTTP MCP. Caddy preserves the normal `Authorization` request header through `reverse_proxy`; do not replace the OAuth bearer token with the operator bearer at the proxy.
