...
pub const STATE_SCHEMA_VERSION: u32 = 34;
...
mod tests {
    ...
    fn identity_has_no_runtime_secret_material() {
        ...
        assert_eq!(identity.state_schema_version, 34);
        ...
        assert!(
            identity
                .capabilities
                .contains(&"mcp-extension-invocation-audit")
        );
        ...
        assert!(identity.capabilities.contains(&"harness-job-runtime"));
        assert!(
            identity
                .capabilities
                .contains(&"conversation-workspace-scope")
        );
        assert!(
            identity
                .capabilities
                .contains(&"harness-recovery-checkpoints")
        );
