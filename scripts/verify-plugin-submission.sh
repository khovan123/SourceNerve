...
readarray -t mcp_parts < <(python3 - "$MCP_URL" "$BASE_URL" <<'PY'
import sys
from urllib.parse import urlsplit

mcp = urlsplit(sys.argv[1])
control = urlsplit(sys.argv[2])
if mcp.scheme != "https" or not mcp.hostname or mcp.username or mcp.password or mcp.query or mcp.fragment:
    raise SystemExit("MCP URL must be credential-free HTTPS without query or fragment")
...
if origin == control_origin:
    raise SystemExit("MCP URL must be installation-specific; the central control-plane origin does not serve /mcp")
print(origin)
print(mcp.path.rstrip("/"))
PY
)
[[ "${#mcp_parts[@]}" -eq 2 ]] || fail "unable to parse installation-specific MCP URL"
MCP_ORIGIN="${mcp_parts[0]}"
...
grep -Fq "resource_metadata=\"${METADATA_URL}\"" "$headers"   || fail "MCP Bearer challenge is missing the installation-scoped protected-resource metadata URL"
...
if [[ -n "${SOURCENERVE_OPENAI_APPS_CHALLENGE:-}" ]]; then
  ...
  [[ "$challenge" == "$SOURCENERVE_OPENAI_APPS_CHALLENGE" ]]     || fail "installation MCP challenge endpoint does not return the exact configured token"
