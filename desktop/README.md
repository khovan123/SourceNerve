...
`SOURCENERVE_MACOS_SIGN_IDENTITY` enables the current Electron Packager/macOS signer with its supported signing defaults. All three Apple notarization values must be present before notarization is enabled. `assets/entitlements.mac.plist` is retained as the reviewed baseline/template for the production signing policy rather than being forced onto every helper binary from this packaging issue; helper-specific entitlements, hardened-runtime verification, stapling, notarized DMG policy and Windows Authenticode policy are completed in #82. Because the native DMG step consumes the already packaged `.app`, it preserves the packaged app signature rather than introducing a second app-signing path.
