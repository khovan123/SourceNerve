...
if (packaged.length === 0) {
  throw new Error(
    `packaged SourceNerve daemon not found under out/**/${expectedSuffix}`,
  );
}
...
for (const candidate of packaged) {
  ...
  if (!metadata.isFile() || metadata.size === 0) {
    throw new Error(`packaged SourceNerve daemon is invalid: ${candidate}`);
  }
  if (process.platform !== "win32" && (metadata.mode & 0o111) === 0) {
    throw new Error(`packaged SourceNerve daemon is not executable: ${candidate}`);
  }
}

console.log(`verified ${packaged.length} packaged SourceNerve daemon resource(s)`);
