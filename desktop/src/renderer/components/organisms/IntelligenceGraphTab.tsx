...
export function IntelligenceGraphTab({
  ...
  return (
    <div className="space-y-4">
      <SurfaceCard title="Graph health" eyebrow="Indexed parse and edge coverage" compact actions={graphStatus ? <StatusPill dot tone="ready">Graph v{graphStatus.graphVersion}</StatusPill> : <StatusPill tone="warning">Unavailable</StatusPill>}>
        {graphStatus ? (
        ...
        ) : <EmptyState icon={Network} title="Graph unavailable" description="Index the workspace to populate symbols and graph edges." compact />}
      </SurfaceCard>

      <SurfaceCard title="Symbol explorer" eyebrow="Definitions, edges and impact" description="Search symbols in the master pane, then inspect relationships and bounded traces in the detail pane.">
        <div className="space-y-4">
          <div className="grid gap-3 rounded-xl border border-border bg-muted/15 p-3 lg:grid-cols-[minmax(0,1fr)_14rem_auto] lg:items-end">
            <label className="grid gap-1.5"><span className="text-xs font-medium text-muted-foreground">Symbol query</span><input className={controlClass} value={query} maxLength={4096} onChange={(event) => onQuery(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && query.trim() && busy !== "symbols") onSearch(); }} placeholder="router, WorkspaceManager, index_workspace…" /></label>
          ...
          <div className="grid min-h-72 gap-4 xl:grid-cols-[minmax(17rem,0.72fr)_minmax(0,1.28fr)]">
            <section className="min-w-0 rounded-xl border border-border bg-background/30 p-3" aria-label="Symbol search results">
              <div className="mb-2 flex items-center justify-between"><h3 className="text-xs font-semibold text-foreground">Symbols</h3><StatusPill tone="neutral">{search?.symbols.length ?? 0}</StatusPill></div>
              <div className="max-h-[42rem] space-y-2 overflow-auto overscroll-contain pr-1" tabIndex={0}>
                {search?.symbols.map((symbol) => (
                  <button key={symbol.symbolKey} aria-pressed={selectedSymbol?.symbolKey === symbol.symbolKey} className={`w-full rounded-xl border px-3 py-3 text-left transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/25 ${selectedSymbol?.symbolKey === symbol.symbolKey ? "border-primary/35 bg-primary/[0.07] shadow-sm" : "border-border bg-card/60 hover:border-primary/15 hover:bg-muted/35"}`} type="button" onClick={() => onSelectSymbol(symbol)}>
                    <strong className="block break-words text-xs text-foreground">{symbol.qualifiedName}</strong>
                    <span className="mt-1 block break-all font-mono text-[10px] leading-4 text-muted-foreground">{symbol.kind} · {symbol.path}{symbol.startLine ? `:${symbol.startLine}` : ""}</span>
                  </button>
                ))}
                {!search ? <EmptyState icon={Search} title="Find a symbol" description="Search the current graph to inspect definitions, relationships and impact." compact /> : null}
                {search?.symbols.length === 0 ? <EmptyState icon={Search} title="No symbols found" description="Try a broader name or remove the optional kind filter." compact /> : null}
              </div>
            </section>

            <section className="min-w-0 rounded-xl border border-border bg-muted/15 p-4" aria-label="Selected symbol detail">
              {selectedSymbol ? (
                <div className="space-y-4">
                  <div>
                    <div className="flex items-start gap-2"><Network className="mt-0.5 size-4 shrink-0 text-muted-foreground" aria-hidden="true" /><div className="min-w-0"><h3 className="break-words text-sm font-semibold text-foreground">{selectedSymbol.qualifiedName}</h3><p className="mt-1 select-all break-all font-mono text-[11px] leading-5 text-muted-foreground">{selectedSymbol.signature ?? `${selectedSymbol.kind} · ${selectedSymbol.path}`}</p></div></div>
                  ...
                  {trace ? <div className="max-h-80 space-y-2 overflow-auto overscroll-contain" tabIndex={0}>{trace.nodes.map((node, index) => <article className="rounded-xl border border-border bg-card/70 p-3" key={`${node.symbol.symbolKey}-${node.distance}-${index}`}><div className="flex flex-wrap items-center justify-between gap-2"><strong className="break-words text-xs text-foreground">{node.symbol.qualifiedName}</strong><span className="text-[10px] text-muted-foreground">distance {node.distance} · {node.via} · {node.source}</span></div><p className="mt-1 break-all font-mono text-[10px] text-muted-foreground">{node.symbol.path}{node.symbol.startLine ? `:${node.symbol.startLine}` : ""}</p><ActionButton variant="ghost" size="sm" onClick={() => onSelectSymbol(node.symbol)} className="mt-1">Inspect symbol</ActionButton></article>)}</div> : null}
                </div>
              ) : <EmptyState icon={Network} title="Select a symbol" description="Incoming/outgoing edges, traces and source preview actions will appear here." compact />}
            </section>
          </div>
        </div>
      </SurfaceCard>
...
function EdgeList({ title, edges, onSelect }: { title: string; edges: IntelligenceSymbolContext["incoming"]; onSelect(symbol: IntelligenceSymbolView): void }) {
  return <section className="rounded-xl border border-border bg-card/60 p-3"><div className="mb-2 flex items-center justify-between"><h4 className="text-[11px] font-semibold text-foreground">{title}</h4><StatusPill tone="neutral">{edges.length}</StatusPill></div><div className="max-h-52 space-y-1 overflow-auto overscroll-contain" tabIndex={0}>{edges.slice(0, 50).map((edge, index) => <button className="w-full rounded-lg px-2 py-2 text-left transition hover:bg-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/25" type="button" key={`${edge.symbol.symbolKey}-${edge.edgeType}-${index}`} onClick={() => onSelect(edge.symbol)}><strong className="block break-words text-[10px] text-foreground">{edge.symbol.qualifiedName}</strong><span className="mt-0.5 block text-[9px] text-muted-foreground">{edge.edgeType} · {formatIntelligenceScore(edge.confidence)} · {edge.source}</span></button>)}</div></section>;
}
