...
const controlClass =
  "h-10 w-full rounded-xl border border-border bg-background/70 px-3 text-sm text-foreground outline-none transition focus:border-primary/45 focus:ring-2 focus:ring-primary/10 disabled:cursor-not-allowed disabled:opacity-50";
        ...
        <Field label="Repository" error={fieldErrors.repository}>
          <div className="grid gap-2 rounded-xl border border-border bg-muted/35 p-3 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center">
            <code
              ...
              title={draft.root}
            >
              {compactWorkspacePath(draft.root)}
            </code>
            <ActionButton
              variant="secondary"
              size="md"
        ...
        <div className="grid items-start gap-4 md:grid-cols-2">
          ...
          <Field label="Access" error={fieldErrors.access}>
            <select
              ...
              onChange={(event) =>
                onChange({
                  ...draft,
                  access: event.target.value as WorkspaceAccess,
                })
              }
            >
              ...
              <option
                value="read-write"
                disabled={draft.selection?.localWritable === false}
              >
                Read-write
              </option>
            </select>
          ...
          <Field label="Remote" error={fieldErrors.remote}>
            <select
              ...
              value={draft.remote}
              onChange={(event) =>
                onChange({ ...draft, remote: event.target.value })
              }
            >
              {remotes.map((remote) => (
          ...
          <div className="md:col-span-2">
            <Field label="Default branch" error={fieldErrors.defaultBranch}>
              <input
                ...
                maxLength={256}
                onChange={(event) =>
                  onChange({ ...draft, defaultBranch: event.target.value })
                }
              />
            </Field>
          </div>
...
function Field({
  ...
  return (
    <label className="grid min-w-0 content-start gap-1.5 text-sm">
      ...
      {children}
      {error ? (
        <small className="text-xs leading-5 text-danger">{error}</small>
      ) : null}
    </label>
  );
}
