...
describe("SourceNerve public plugin registry", () => {
  it("stages all bundled SourceNerve skills for ChatGPT/Codex", async () => {
    ...
    vi.stubGlobal("fetch", vi.fn(async (input: string | URL | Request) => {
      ...
      if (url.endsWith("/skills/chatgpt-review-loop/SKILL.md")) return new Response("# ChatGPT Review Loop\n", { status: 200 });
    ...
    try {
      ...
      expect(await readFile(path.join(staged.sourcePath, "skills", "chatgpt-review-loop", "SKILL.md"), "utf8")).toContain("ChatGPT Review Loop");
      ...
      expect(requested.some((url) => url.endsWith("/skills/chatgpt-review-loop/SKILL.md"))).toBe(true);
