...
export class ProviderManager {
  ...
  async initialize(): Promise<ProviderAccountView[]> {
    ...
    for (const provider of ["github", "gitlab"] as const) {
      ...
      try {
        ...
        this.accounts.set(provider, stored);
  ...
  async connect(provider: GitProvider): Promise<ProviderAccountView> {
    ...
    const verificationComplete = payload.verificationUriComplete
      ...
      : verificationUri;
    const intervalSeconds = boundedInterval(payload.intervalSeconds);
  ...
  async validateRepository(provider: GitProvider, repository: string): Promise<ProviderRepositorySummary> {
    ...
    if (provider === "github") {
      ...
      const response = await this.apiRequest(provider, token, url.toString());
      return parseGitHubRepository(response.value);
    }
    const url = new URL(`/api/v4/projects/${encodeURIComponent(repository)}`, "https://gitlab.com");
    const response = await this.apiRequest(provider, token, url.toString());
    return parseGitLabRepository(response.value);
  }
  ...
  private async pollDeviceLogin(pending: PendingDeviceLogin): Promise<void> {
    ...
    while (this.now() < pending.expiresAt) {
      ...
      const response = await this.formRequest(profile.tokenUrl, {
        client_id: profile.clientId,
  ...
  private async formRequest(url: string, fields: Record<string, string>): Promise<JsonResponse> {
  ...
  private async fetchJson(url: string, init: RequestInit): Promise<JsonResponse> {
    ...
    try {
      ...
      if (!response.ok) {
        const message = providerHttpError(response.status);
...
function boundedInterval(value: number): number {
  return Math.min(MAX_POLL_INTERVAL_SECONDS, Math.max(MIN_POLL_INTERVAL_SECONDS, Math.ceil(value)));
}

function validAccessToken(value: unknown): value is string {
  return typeof value === "string" && value.length >= 20 && value.length <= MAX_TOKEN_BYTES && /^[\x21-\x7e]+$/.test(value);
}
