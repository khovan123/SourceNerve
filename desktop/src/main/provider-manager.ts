...
export class ProviderManager {
  ...
  states(): ProviderAccountView[] {
    return (["github", "gitlab"] as const).map((provider) => this.state(provider));
  }
  ...
  state(provider: GitProvider): ProviderAccountView {
    ...
    return { provider, status: error ? "error" : "disconnected", baseUrl: profile.apiBaseUrl, error };
  }
  ...
  async listRepositories(provider: GitProvider): Promise<ProviderRepositorySummary[]> {
    ...
    return provider === "github" ? this.listGitHubRepositories(token) : this.listGitLabRepositories(token);
  }
  ...
  async validateRepository(provider: GitProvider, repository: string): Promise<ProviderRepositorySummary> {
    ...
    if (provider === "github") {
      ...
      if (segments.length !== 2) throw new Error("GitHub repository slug must be owner/repository");
      const url = new URL(`/repos/${encodeURIComponent(segments[0])}/${encodeURIComponent(segments[1])}`, `${this.profile(provider).apiBaseUrl}/`);
      return parseGitHubRepository((await this.apiRequest(provider, token, url.toString())).value);
    }
    const url = new URL(`/api/v4/projects/${encodeURIComponent(repository)}`, "https://gitlab.com");
    return parseGitLabRepository((await this.apiRequest(provider, token, url.toString())).value);
  }
  ...
  async validateGitTransport(workspaceId: string): Promise<GitTransportValidation> {
    ...
    const workspace = (await this.workspaceManager.list()).find((item) => item.id === workspaceId);
    if (!workspace) throw new Error(`workspace '${workspaceId}' is not registered`);
    if (!workspace.validation.valid) throw new Error(`workspace '${workspaceId}' must be repaired before Git transport validation`);

    const remoteUrl = await git(workspace.root, ["remote", "get-url", workspace.remote]);
    const transport = classifyGitTransport(remoteUrl.trim());
    try {
      await git(workspace.root, ["ls-remote", "--exit-code", workspace.remote, "HEAD"], true);
      return { workspace: workspaceId, ready: true, transport, message: `Non-interactive ${transport.toUpperCase()} Git transport is ready.` };
    } catch {
      return {
  ...
  private async pollDeviceLogin(pending: PendingDeviceLogin): Promise<void> {
    ...
    while (this.now() < pending.expiresAt) {
      ...
      const current = this.pending.get(pending.provider);
      ...
      if (typeof response.value.error === "string") {
        ...
        if (error === "access_denied") throw new Error(`${providerLabel(pending.provider)} authorization was denied`);
        if (error === "device_flow_disabled") throw new Error("GitHub device flow is disabled for the configured OAuth application");
        if (error === "unsupported_grant_type") throw new Error(`${providerLabel(pending.provider)} device authorization is not supported`);
        throw new Error(`${providerLabel(pending.provider)} authorization failed (${safeProviderError(error)})`);
      }
  ...
  private async fetchAccount(provider: GitProvider, token: string): Promise<{ login: string; name?: string; providerUserId: string }> {
    const profile = this.profile(provider);
    const url = provider === "github"
      ? new URL("/user", `${profile.apiBaseUrl}/`).toString()
      : new URL("/api/v4/user", "https://gitlab.com").toString();
    const response = await this.apiRequest(provider, token, url);
    ...
    if (typeof login !== "string" || !boundedText(login, 256)) throw new Error(`${providerLabel(provider)} account login is invalid`);
    const id = response.value.id;
    if ((typeof id !== "number" && typeof id !== "string") || String(id).length > 128) throw new Error(`${providerLabel(provider)} account id is invalid`);
    const name = typeof response.value.name === "string" && boundedText(response.value.name, 256) ? response.value.name : undefined;
  ...
  private async listGitHubRepositories(token: string): Promise<ProviderRepositorySummary[]> {
    ...
    for (let page = 1; page <= MAX_PAGES && results.length < MAX_REPOSITORIES; page += 1) {
      const url = new URL("/user/repos", `${this.profile("github").apiBaseUrl}/`);
  ...
  private async formRequest(url: string, fields: Record<string, string>, allowOAuthError = false): Promise<JsonResponse> {
  ...
  private async apiRequest(provider: GitProvider, token: string, url: string): Promise<JsonResponse> {
    ...
    const apiOrigin = new URL(profile.apiBaseUrl).origin;
    if (target.protocol !== "https:" || target.origin !== apiOrigin || target.username || target.password || target.hash) {
  ...
  private async fetchJson(url: string, init: RequestInit, allowOAuthError = false): Promise<JsonResponse> {
    ...
    try {
      ...
      if (!response.ok) {
        if (allowOAuthError && isRecord(value) && typeof value.error === "string" && /^[A-Za-z0-9._-]{1,128}$/.test(value.error)) {
  ...
  private publish(provider: GitProvider, state: string, message?: string): void {
    this.onEvent?.({ type: "state", component: "git", state, message: message ? `${providerLabel(provider)}: ${message}` : providerLabel(provider) });
  }
}
...
async function readMetadata(filePath: string): Promise<StoredProviderAccount[]> {
  try {
    ...
    if (value.schemaVersion !== METADATA_SCHEMA_VERSION || !Array.isArray(value.accounts)) throw new Error("provider metadata file is invalid");
...
function validateStoredAccount(value: unknown): StoredProviderAccount {
  if (!isRecord(value) || (value.provider !== "github" && value.provider !== "gitlab")) throw new Error("invalid provider account metadata");
  if (typeof value.login !== "string" || !boundedText(value.login, 256)) throw new Error("invalid provider account login");
  if (typeof value.providerUserId !== "string" || !boundedText(value.providerUserId, 128)) throw new Error("invalid provider user id");
  if (typeof value.baseUrl !== "string" || !isCredentialFreeHttps(value.baseUrl)) throw new Error("invalid provider API base URL");
  if (typeof value.connectedAt !== "number" || !Number.isFinite(value.connectedAt)) throw new Error("invalid provider connected timestamp");
  return {
...
function parseDeviceAuthorization(value: unknown): {
  ...
  if (!boundedTokenField(deviceCode, 1024) || !boundedTokenField(userCode, 128) || typeof verificationUri !== "string") {
  ...
  if (!Number.isFinite(expiresIn) || expiresIn < 60 || expiresIn > 3600) throw new Error("provider device authorization expiry is invalid");
  if (!Number.isFinite(interval) || interval < 1 || interval > MAX_POLL_INTERVAL_SECONDS) throw new Error("provider device polling interval is invalid");
  return {
    deviceCode,
    userCode,
    verificationUri,
    verificationUriComplete: typeof value.verification_uri_complete === "string" ? value.verification_uri_complete : undefined,
...
function validateVerificationUri(value: string, profile: DeviceProviderProfile): string {
  ...
  if (url.protocol !== "https:" || url.origin !== expectedOrigin || url.username || url.password || url.hash) {
...
function parseAccessToken(value: Record<string, unknown>): string {
  ...
  if (!validAccessToken(token) || (typeof tokenType === "string" && tokenType.toLowerCase() !== "bearer")) {
...
function parseGitHubRepository(value: unknown): ProviderRepositorySummary {
  if (!isRecord(value) || typeof value.full_name !== "string" || !validRepositorySlug(value.full_name)) throw new Error("GitHub repository item is invalid");
  if (typeof value.name !== "string" || !boundedText(value.name, 256)) throw new Error("GitHub repository name is invalid");
  const permissions = isRecord(value.permissions) ? value.permissions : {};
  return {
    provider: "github",
    slug: value.full_name,
    name: value.name,
    defaultBranch: typeof value.default_branch === "string" && boundedText(value.default_branch, 256) ? value.default_branch : undefined,
...
function parseGitLabRepository(value: unknown): ProviderRepositorySummary {
  if (!isRecord(value) || typeof value.path_with_namespace !== "string" || !validRepositorySlug(value.path_with_namespace)) throw new Error("GitLab project item is invalid");
  if (typeof value.name !== "string" || !boundedText(value.name, 256)) throw new Error("GitLab project name is invalid");
  const permissions = isRecord(value.permissions) ? value.permissions : {};
...
function providerLabel(provider: GitProvider): string { return provider === "github" ? "GitHub" : "GitLab"; }
function boundedInterval(value: number): number { return Math.min(MAX_POLL_INTERVAL_SECONDS, Math.max(MIN_POLL_INTERVAL_SECONDS, Math.ceil(value))); }
function validAccessToken(value: unknown): value is string { return typeof value === "string" && value.length >= 20 && value.length <= MAX_TOKEN_BYTES && /^[\x21-\x7e]+$/.test(value); }
function boundedTokenField(value: unknown, max: number): value is string { return typeof value === "string" && value.length >= 1 && value.length <= max && /^[A-Za-z0-9._~+/=-]+$/.test(value); }
...
function boundedText(value: string, max: number): boolean { return value.length >= 1 && value.length <= max && !/[\u0000-\u001f\u007f]/.test(value); }
function safeHttpsField(value: unknown, label: string): string {
  if (typeof value !== "string" || !isCredentialFreeHttps(value)) throw new Error(`${label} is invalid`);
  return value;
}
...
function safeProviderError(value: string): string { return /^[A-Za-z0-9._-]{1,128}$/.test(value) ? value : "provider_error"; }
