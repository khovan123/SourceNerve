...
export function readPluginSetupFields(profileValue: unknown): PluginSetupFields {
  ...
  const description = firstString([
    plugin?.description,
    product?.description,
  ], "plugin description", 1, 2_048);
  const publicMcpResource = absoluteHttps(firstString([
    publicMcp?.resource,
    plugin?.publicMcpResource,
    plugin?.mcpResource,
  ], "public MCP resource", 1, 2_048), "public MCP resource");
  const oauthIssuer = issuerUrl(firstString([
    auth0?.issuer,
    plugin?.oauthIssuer,
  ], "OAuth issuer", 1, 2_048));
  const oauthResource = absoluteHttps(firstString([
    auth0?.resource,
    auth0?.audience,
    plugin?.oauthResource,
    publicMcpResource,
  ], "OAuth resource", 1, 2_048), "OAuth resource");
  const oauthScopes = stringArray(
    auth0?.scopes ?? plugin?.oauthScopes ?? plugin?.scopes,
    "OAuth scopes",
  );
  const privacyUrl = absoluteHttps(firstString([
    plugin?.privacyUrl,
    legal?.privacyUrl,
    product?.privacyUrl,
  ], "privacy URL", 1, 2_048), "privacy URL");
  const termsUrl = absoluteHttps(firstString([
    plugin?.termsUrl,
    legal?.termsUrl,
    product?.termsUrl,
  ], "terms URL", 1, 2_048), "terms URL");
  const supportUrl = absoluteHttps(firstString([
    plugin?.supportUrl,
    legal?.supportUrl,
    product?.supportUrl,
  ], "support URL", 1, 2_048), "support URL");
  const iconCandidate = optionalFirstString([
    plugin?.iconUrl,
    product?.iconUrl,
  ], 2_048);
  const chatgptCandidate = optionalFirstString([
    plugin?.chatgptSetupUrl,
    plugin?.chatgptUrl,
    product?.chatgptSetupUrl,
  ], 2_048);
...
function stringArray(value: unknown, label: string): string[] {
  if (!Array.isArray(value) || value.length < 1 || value.length > 32) {
    throw new Error(`${label} is missing from the packaged product profile`);
  }
  const result = value.map((item) => {
    if (typeof item !== "string" || item.length < 1 || item.length > 256 || /[\u0000-\u0020\u007f]/.test(item)) {
...
function issuerUrl(value: string): string {
  ...
  if (parsed.protocol !== "https:" || parsed.username || parsed.password || parsed.search || parsed.hash) {
    throw new Error("OAuth issuer must be credential-free HTTPS");
  }
...
function optionalFirstString(values: unknown[], max: number): string | undefined {
  for (const value of values) {
    if (typeof value === "string" && value.length > 0 && value.length <= max && !/[\u0000-\u001f\u007f]/.test(value)) return value;
  }
  return undefined;
}
