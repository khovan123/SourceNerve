...
export class McpExtensionManager {
  ...
  async initialize(): Promise<void> {
    ...
    for (const item of health) {
      ...
      await this.client.materializeCredential(item.extension.id, credential).catch((error) => {
        this.emit("warn", `MCP extension ${item.extension.id} credential restore deferred: ${safeMessage(error)}`);
      });
    }
  }
  ...
  async install(input: McpExtensionInstallInput): Promise<McpExtensionView> {
    ...
    try {
      ...
      if (key) await this.secretStore.deleteOpaque(key).catch(() => undefined);
      throw error;
    }
    this.emit("info", `Installed MCP extension ${input.id}; tools remain blocked until explicitly permitted`);
  ...
  async remove(extensionId: string): Promise<{ removed: boolean }> {
    ...
    await this.secretStore.deleteOpaque(secretKey(extensionId)).catch(() => undefined);
  ...
  async updateToolPolicy(input: McpExtensionToolPolicyInput): Promise<McpExtensionToolView> {
    ...
    if (!input.toolName || input.toolName.length > MAX_TOOL_NAME) throw new Error("MCP tool name is invalid");
    ...
    this.emit("info", `Updated ${tool.publicName} policy to ${tool.enabled ? input.approval : "disabled"}`);
    return tool;
  }

  async setCredential(input: McpExtensionCredentialInput): Promise<{ configured: true }> {
    validateExtensionId(input.extensionId);
    if (!input.credential || input.credential.length > 32 * 1024 || input.credential.includes("\0")) {
      throw new Error("MCP extension credential is invalid");
    }
    const current = await this.requireView(input.extensionId);
    if (current.authType === "none") throw new Error("This MCP extension does not use credentials");
  ...
  async approveNext(publicTool: string): Promise<McpExtensionApprovalResult> {
    ...
    this.emit("info", `Approved one execution of ${publicTool} for ${value.expiresInSeconds}s`);
    return value;
  }
...
function validateInstallInput(input: McpExtensionInstallInput): void {
  ...
  if (!input.namespace || input.namespace === "sourcenerve" || !/^[a-z0-9_-]{1,48}$/.test(input.namespace)) {
...
function parseHealthItem(value: unknown): HealthItem {
  if (!isRecord(value) || !isRecord(value.extension)) throw new Error("MCP extension health item is invalid");
  const extension = parseExtension(value.extension);
  if (!nonNegativeInteger(value.discovered_tools) || !nonNegativeInteger(value.exposed_tools) || typeof value.credential_materialized !== "boolean") {
...
function parseExtension(value: Record<string, unknown>): ExtensionRecordShape {
  ...
  if (
    ...
    typeof value.namespace !== "string" || typeof value.source !== "string" || !isAuthType(authType) ||
    !isStatus(status) || typeof value.enabled !== "boolean" || typeof value.required !== "boolean" ||
    typeof value.update_channel !== "string" || !Number.isSafeInteger(value.created_at) || !Number.isSafeInteger(value.updated_at) ||
...
function parseTransport(value: unknown): McpExtensionTransport {
  ...
  if (value.transport === "stdio" && typeof value.command === "string" && Array.isArray(value.args) && value.args.every((item) => typeof item === "string")) {
    return { transport: "stdio", command: value.command, args: [...value.args] };
  }
...
function parseTool(value: unknown): McpExtensionToolView {
  ...
  if (
    typeof value.extension_id !== "string" || typeof value.original_name !== "string" ||
    typeof value.public_name !== "string" || typeof value.schema_hash !== "string" ||
    typeof value.policy.enabled !== "boolean" || !isApproval(value.policy.approval)
  ...
  return {
    ...
    classification: {
      ...(typeof classification.read_only === "boolean" ? { readOnly: classification.read_only } : {}),
      ...(typeof classification.destructive === "boolean" ? { destructive: classification.destructive } : {}),
      ...(typeof classification.idempotent === "boolean" ? { idempotent: classification.idempotent } : {}),
      ...(typeof classification.open_world === "boolean" ? { openWorld: classification.open_world } : {}),
    },
  };
}

function parseRemoved(value: unknown): { removed: boolean } {
  if (!isRecord(value) || typeof value.removed !== "boolean") throw new Error("MCP extension remove response is invalid");
  return { removed: value.removed };
}

function parseApproval(value: unknown): McpExtensionApprovalResult {
  if (!isRecord(value) || typeof value.public_tool !== "string" || value.approved_once !== true || !nonNegativeInteger(value.expires_in_seconds)) {
    throw new Error("MCP extension approval response is invalid");
  }
  return { publicTool: value.public_tool, approvedOnce: true, expiresInSeconds: value.expires_in_seconds };
}
...
function isStatus(value: unknown): value is McpExtensionView["status"] {
  return value === "installed" || value === "enabled" || value === "disabled" || value === "error" || value === "updating";
}
